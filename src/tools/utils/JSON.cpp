...
namespace tools::utils {
    ...
    JSON::JSON(const nlohmann::json& j) : j(j) {}

    JSON::JSON(const char* j) : JSON(string(j)) {}

    JSON::JSON(string jstring) {
        try {
    ...
    nlohmann::json JSON::get_json() const {
        return j;
    }

    bool JSON::isValid(string* error) {
        if (error) *error = _error ? *_error : "";
        return !_error;
    }

    string JSON::dump(int indent, char indent_char) const {
        try {
    ...
    bool JSON::isDefined(string jselector) const {
        try {
    ...
    bool JSON::has(string jselector) const {
        return isDefined(jselector);
    }

    bool JSON::isNull(string jselector) {
        try {
            nlohmann::json::json_pointer ptr = _json_selector(jselector);
    ...
    bool JSON::isArray(string jselector) {
        try {
            nlohmann::json::json_pointer ptr = _json_selector(jselector);
    ...
    bool JSON::isObject(string jselector) {
        try {
            nlohmann::json::json_pointer ptr = _json_selector(jselector);
